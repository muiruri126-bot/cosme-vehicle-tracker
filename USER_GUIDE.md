...
Plan International Kenya | Version 1.0 | February 2026
...
12. Audit Log (Admin)
13. Profile and Password Management
...
|---|:---:|:---:|:---:|
| View Dashboard, Bookings, Calendar, Reports | Yes | Yes | Yes |
| Create a Booking | Yes | Yes | Yes |
| Cancel a Booking (Pending/Approved) | Yes | Yes | Yes |
| Start / End a Trip | Yes | Yes | No |
| Approve Bookings | Yes | No | No |
| Assign a Driver to a Booking | Yes | No | No |
| Add / Edit / Delete Vehicles | Yes | No | No |
| Add / Complete / Cancel / Delete Maintenance | Yes | No | No |
| Manage Users (List, Edit, Delete, Reset Password) | Yes | No | No |
...
- Username allows only: a-z, A-Z, 0-9, dot (.), underscore (_).
...
**Validation Checks - Password Reset:**
...
**Validation Checks - Cancellation:**
...
Note: Only Admins and Drivers can start a trip. Requesters do not have access to this function.
...
**Validation Checks - Trip Start:**

- Only Admins and Drivers can start a trip (Requesters are blocked).
...
Note: Only Admins and Drivers can end a trip. Requesters do not have access to this function.
...
| Odometer End (km) | Yes | Must be greater than or equal to Odometer Start |
...
**Validation Checks - Trip End:**

- Only Admins and Drivers can end a trip (Requesters are blocked).
...
- Distance is automatically calculated as: Odometer End minus Odometer Start.
...
**Validation Checks - Edit Vehicle:**
...
1. Navigate to Reports then Vehicle Trip Report.
2. Optionally filter by:
   - Vehicle (dropdown)
   - Date Range (From / To)
3. Click Generate Report.
4. View completed trip details: dates, routes, distance, fuel, cost.
5. Click Export to Excel to download a .xlsx file with formatted data and totals.


### 9.2 Budget and Fuel Cost Report

1. Navigate to Reports then Budget Report.
...
**Validation Checks - Change Password:**
...
| Area | Rule | Type |
|---|---|---|
| Registration | Username is at least 3 chars, alphanumeric + dots/underscores | Server + Client |
| Registration | Username must be unique | Server |
| Registration | Email must be valid and unique | Server + Client |
| Registration | Password is at least 6 characters | Server + Client |
...
| Login | Rate limited: 10 per minute | Server |
| Password Reset | Token expires after 24 hours | Server |
| Password Reset | Rate limited: 5 per minute | Server |
| Change Password | Current password must be correct | Server |
| Change Password | New password cannot equal current password | Server |
| Change Password | New password is at least 6 characters | Server + Client |
...
| Trip Start | Odometer must be 0 or greater | Server + Client |
| Trip Start | Only for approved bookings without trip | Server |
| Trip Start | Only admins and drivers can start | Server |
| Trip End | End time must be after start time | Server + Client |
| Trip End | Only admins and drivers can end | Server |
| Trip End | Odometer End must be at least Odometer Start | Server + Client |
| Trip End | Fuel Used and Cost must be 0 or greater (if provided) | Server + Client |
| Vehicle | Registration is at least 3 chars, unique | Server + Client |
| Vehicle | Make and Model required | Server + Client |
| Maintenance | Vehicle, Type, Description, Date required | Server + Client |
| Maintenance | Cost must be 0 or greater (if provided) | Server + Client |
...
A: Go to Reports, then Vehicle Trip Report, set your filters, and click Export to Excel.
